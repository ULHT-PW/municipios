import type { Metadata } from "next";
import localFont from "next/font/local";
import "./globals.css";
import Link from "next/link";

const geistSans = localFont({
  src: "./fonts/GeistVF.woff",
  variable: "--font-geist-sans",
  weight: "100 900",
});
const geistMono = localFont({
  src: "./fonts/GeistMonoVF.woff",
  variable: "--font-geist-mono",
  weight: "100 900",
});

export const metadata: Metadata = {
  title: "Municipios",
  description: "Next.js app",
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang="en">
      <body
        className={`${geistSans.variable} ${geistMono.variable} antialiased
          h-[100vh]
          flex flex-col justify-center items-center
          bg-purple-900
        `}
      >
        
        <header className=" 
              w-[95vw] max-w-[700px] pb-3
              flex flex-row justify-between items-center
              gap-5
              text-white 
        ">
          <h1 className="text-4xl font-bold">Municípios</h1>
          <nav className="flex flex-row gap-4">
            <Link  className="hover:underline" href="/">Home</Link>
            <Link className="hover:underline" href="/municipalities">Municípios</Link>
            <Link className="hover:underline" href="/lines">Linhas</Link>
            {/* <Link className="hover:underline" href="/counter">Contador</Link>             */}
            <Link className="hover:underline" href="/about">Sobre</Link>            
          </nav>
        </header>

        <main className="h-[70vh] w-[95vw] max-w-[700px] p-5 rounded-xl bg-white overflow-auto">{children}</main>
        <footer className="text-sm text-white pt-4">2025, DEISI, Lusófona</footer>

      </body>
    </html>
  );
}
